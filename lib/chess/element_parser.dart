...
class ElementParser {
  ...
  List<ParsedElement> parseWordBlock(
    ...
    for (final blob in blobResults) {
      ...
      final pixels = _cropAndResizeBlob(rendered.gray, rendered.width,
          rendered.height, pdfBounds, pageHeight, renderScale);
      final features = HogExtractor.extract(pixels, 32, 32);

      // Classify as text vs figurine
      final notFigurineConf = notAFigurineClassifier.classifyAsNotFigurine(features) ?? 0.0;
  ...
  static Float32List _cropAndResizeBlob(
    ...
    final pw = ((bounds.right - bounds.left) * scale).round().clamp(1, imgW - px0);
    final ph = ((bounds.top - bounds.bottom) * scale).round().clamp(1, imgH - py0);
